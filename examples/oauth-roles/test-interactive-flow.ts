...
async function main() {
  ...
  let resourceServer: HttpResourceServerTransport | null = null;
  ...
  const cleanup = async () => {
    if (resourceServer) {
      try {
        ...
        console.error('[Cleanup] Error closing resource server:', (e as Error).message);
      }
    }
    if (authServer) {
      try {
  ...
  try {
    ...
    try {
      ...
      console.error('  Hint: Another process may be using port 4000');
    ...
    resourceServer = new HttpResourceServerTransport('2025-06-18', {
      enabled: true,
